...
def melspectrogram(y):
    """
    D = _mel_stft(preemphasis(y))
    S = _amp_to_db(D) - hparams.ref_level_db
    return _normalize(S)
    """
    D = _stft(preemphasis(y))
...
def mfcc(y):
    D = _stft(preemphasis(y))
    S = _amp_to_db(_linear_to_mel(np.abs(D))) - hparams.ref_level_db
    M = librosa.feature.mfcc(sr=hparams.sample_rate, S=S, n_mfcc=hparams.n_mfcc)
    return M
...
def _build_mel_basis():
    return librosa.filters.mel(hparams.sample_rate, hparams.n_fft, n_mels=hparams.n_mels)
